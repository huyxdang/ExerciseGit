...
const ACTIVITY_RETRY_DELAYS_MS = [1000, 3000, 7000];
const TOKEN_RETRY_DELAYS_MS = [2000, 10000, 30000, 60000];
const ERROR_BODY_PREVIEW_CHARS = 700;

class TransientStravaBlockError extends Error {
  constructor(message) {
    super(message);
    this.name = "TransientStravaBlockError";
  }
}
...
function truncateText(text, maxChars = ERROR_BODY_PREVIEW_CHARS) {
...
function looksLikeCloudFrontBlock(text) {
  const normalized = text.toLowerCase();
  return normalized.includes("generated by cloudfront") || normalized.includes("request blocked");
}

function allowStaleOnStravaBlock() {
  return ["1", "true", "yes"].includes((process.env.ALLOW_STALE_ON_STRAVA_BLOCK ?? "").toLowerCase());
}
...
async function retryReasonForResponse(response, { retryCloudFrontBlocks = false } = {}) {
  if (RETRYABLE_STATUS_CODES.has(response.status)) {
    return `failed with retryable status ${response.status}`;
  }

  if (retryCloudFrontBlocks && response.status === 403) {
    const body = await response.clone().text();
    if (looksLikeCloudFrontBlock(body)) {
      return "was blocked by CloudFront";
    }
  }
...
async function fetchWithRetry(
  ...
  label,
  {
    retryDelaysMs = ACTIVITY_RETRY_DELAYS_MS,
    retryCloudFrontBlocks = false,
    fetchFn = fetch,
    delayFn = delay,
  } = {}
) {
  ...
  for (let attempt = 0; attempt <= retryDelaysMs.length; attempt++) {
    try {
      ...
      const retryReason = await retryReasonForResponse(response, { retryCloudFrontBlocks });
      if (!retryReason) {
        return response;
      }

      lastError = new Error(`${label} ${retryReason}`);
    ...
    console.log(`${lastError.message}; retrying in ${waitMs / 1000}s...`);
...
async function refreshAccessToken({ fetchFn = fetch, delayFn = delay } = {}) {
  const response = await fetchWithRetry(
    ...
    {
      retryDelaysMs: TOKEN_RETRY_DELAYS_MS,
  ...
  if (!response.ok) {
    const body = await responseTextPreview(response);
    if (response.status === 403 && looksLikeCloudFrontBlock(body)) {
      throw new TransientStravaBlockError(`Strava token refresh was blocked by CloudFront after retries: ${body}`);
    }
...
async function main() {
  ...
  await setGithubOutput("svg_generated", "true");
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch(async (error) => {
    if (error instanceof TransientStravaBlockError && allowStaleOnStravaBlock()) {
      console.warn(
        "::warning::Strava token refresh was blocked by CloudFront after retries. Keeping the existing SVG for this run."
      );
      console.warn(error.message);
      console.warn("Keeping the existing SVG and skipping downstream refresh steps for this run.");
      await setGithubOutput("svg_generated", "false");
      return;
    }
